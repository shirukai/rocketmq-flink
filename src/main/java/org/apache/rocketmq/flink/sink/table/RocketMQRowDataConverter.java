...
public class RocketMQRowDataConverter implements Serializable {
    ...
    public void open() {
        if (rowTypeInfo.getArity() == 1 && rowTypeInfo.getFieldTypes()[0].getTypeClass().equals(byte[].class)) {
            onlyVarbinary = true;
        }
        ...
        if (keyColumns != null) {
            ...
            for (int index = 0; index < keyColumns.length; index++) {
                ...
                checkState(fieldIndex >= 0, String.format("[MetaQConverter] Could not find the message-key column: %s.", keyColumns[index]));
        ...
        if (isDynamicTag && dynamicColumn != null) {
            ...
            checkState(fieldIndex >= 0, String.format("[MetaQConverter] Could not find the tag column: %s.", dynamicColumn));
    ...
    public Message convert(RowData row) {
        ...
        if (onlyVarbinary) {
            ...
            for (int index = 0; index < bodyFieldIndexes.length; index++) {
                values[index] = RowData.createFieldGetter(bodyFieldTypes[index].getLogicalType(), bodyFieldIndexes[index]).getFieldOrNull(row);
            }
            try {
                message.setBody(StringUtils.join(values, fieldDelimiter).getBytes(encoding));
                message.setWaitStoreMsgOK(true);
            } catch (UnsupportedEncodingException e) {
                LOG.error(String.format("Unsupported ''{%s}'' encoding charset. Check the encoding configItem in the DDL.", encoding), e);
            }
        }
        if (hasMetadata) {
